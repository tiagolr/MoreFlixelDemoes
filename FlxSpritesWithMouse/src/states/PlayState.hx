...
class PlayState extends FlxState
{
	...
	override public function create():Void 
	{
		...
		add(spr2);
		spr2.mouseEnabled = true;
		txt2 = new FlxText(spr2.x, spr2.y + 120, 100, "\n\nGlow\n\n\n", 10);
		...
		// Animations
		Actuate.tween(filter2, 1, { blurX:4, blurY:4 } ).repeat().reflect().onUpdate(updateSprite, [txt2]).ease(Linear.easeNone);
	...
	{
		#if flash // Seems that angle is private for other targets that flash, can be accessed with reflect tho.
